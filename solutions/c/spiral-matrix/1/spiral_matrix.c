...
static void spiral_matrix_fill(spiral_matrix_t *spiral){
	int number = 1, limit = spiral->size-1;
	int row = 0, column = 0;

	for(; limit > 0; limit -= 2){
		for(int i = 0; i < limit; i++){
			spiral->matrix[row][column+i] = number++;
		}
		for(int i = 0; i < limit; i++){
			spiral->matrix[row+i][column+limit] = number++;
		}
		for(int i = limit; i > 0; i--){
			spiral->matrix[row+limit][column+i] = number++;
		}
		for(int i = limit; i > 0; i--){
			spiral->matrix[row+i][column] = number++;
		}
		row++; column++;
	}

	if(limit == 0){
		spiral->matrix[row][column] = number;
	}
}
