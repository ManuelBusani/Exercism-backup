...
void clear_buffer(circular_buffer_t *buffer){
	if(!buffer) return;
	buffer->size = 0;
	buffer->add_idx = buffer->capacity-1;
	buffer->rmv_idx = 0;
}

static void generic_write(circular_buffer_t *buffer, buffer_value_t value){
	buffer->add_idx = next_idx(buffer->add_idx, buffer->capacity);
	buffer->values[buffer->add_idx] = value;
}
...
int16_t overwrite(circular_buffer_t *buffer, buffer_value_t value){
	...
	if(buffer->size == buffer->capacity){
		buffer->rmv_idx = next_idx(buffer->rmv_idx, buffer->capacity);
	} else buffer->size++;
...
int16_t read(circular_buffer_t *buffer, buffer_value_t *read_value){
	...
	*read_value = buffer->values[buffer->rmv_idx];
	buffer->rmv_idx = next_idx(buffer->rmv_idx, buffer->capacity);
	buffer->size--;
	return EXIT_SUCCESS;
}